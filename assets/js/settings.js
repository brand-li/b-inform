jQuery(function () {
    ...
    init_select(jQuery(".wpnotif_admim_conf").find("select"));
...
//thisGatewayBox.find(".disotp").hide();
    ...
    jQuery(".wpnotif_admim_conf .updatetabview").on('click', function () {
    ...
    jQuery("#wpnotif_setting_update").on("submit", function () {
        ...
        if (code.length == 0) {
            ...
            wpnotif_showMessage(wpnotifobj.invalidpurchasecode);
        ...
        jQuery.post('https://bridge.unitedover.com/updates/verify.php',
            {
                ...
                if (data == 0) {
                    ...
                    if (data == -1) {
                        wpnotif_showMessage("This purchase code is already being used on another site.");
                    } else wpnotif_showMessage(response.msg);
    ...
    function updateSettings(fd, activate) {
        ...
        jQuery.ajax({
            ...
            error: function () {
                isBusy = false;
                invPC();
                wpnotif_showMessage(wpnotifobj.Error);
            }
        });
    ...
    jQuery(document).on("click", ".wpnotif_call_test_api_btn", function () {
        ...
        if (mobile.length == 0 || !jQuery.isNumeric(mobile) || countrycode.length == 0 || !jQuery.isNumeric(countrycode)) {
            wpnotif_showMessage(wpnotifobj.validnumber);
            return false;
        }
...
function wpnotif_showMessage(message) {
