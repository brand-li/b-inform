...
/*
 * Plugin Name: B-inform: SMS & WhatsApp Notifications
 * Description: Send SMS or WhatsApp message to your customers or inform them about their online order updates.
 * Version: 1.8
 * Plugin URI: https://github.com/brand-li/b-inform/
...
final class WPNotif
{
    ...
    public static function add_wp_footer()
    {
        ...
        <div class="wpnotif-box">
            <div class="wpnotif-list-head">
                <?php esc_attr_e('Pending WhatsApp Web Messages', 'wpnotif'); ?>
                ...
                <div class="wpnotif-list wpnotif-pending-whatsapp-messages"
                    ...
                    <div class="wpnotif-inline wpnoti-float-left wpnotif-whatsapp-message">
                        ...
                        <div class="wpnotif-mobile">
                            ...
                            <span class="wpnotif-msg-time"><?php echo date("h:i A, M d ", $message_time); ?></span>
                        </div>
    ...
    public function save_settings()
    {
        ...
        foreach ($options as $option) {
            ...
            foreach ($customer_settings_inputs as $key => $customer_settings_input) {
                $customer_settings[$key]['enable'] = sanitize_text_field($_POST[$option . '_' . $key]);
                $customer_settings[$key]['msg'] = sanitize_textarea_field($_POST[$option . '_' . $key . '_msg']);
            }
    ...
    public static function enqueue_scripts()
    {
        ...
        wp_enqueue_script('wpnotif-frontend', plugins_url('/assets/js/frontend.min.js', __FILE__), array('jquery', 'scrollTo'), self::get_version());
                ...
                <div class="wpnotif_admin_left_side">
                    <div class="wpnotif_admin_left_side_content">
                        ...
                        <form method="post" autocomplete="off" id="wpnotif_setting_update"
                            ...
                            <button type="submit" class="wpnotif_admin_submit"
    ...
    function wpnotif_show_digcountrygateways_settings()
    {
        ...
        echo '<ul class="wpnotif-countrylist" style="display: none;">';
        $this->display_country_list('', 'li');
        echo '</ul>';

    }

    private function display_country_list($selected_country, $wrap = 'option')
    {
        ...
        foreach ($countryList as $key => $value) {
            ...
            echo '<' . $wrap . ' value="' . $value . '" data-country="' . strtolower($key) . '" ' . $sel . '>' . $key . ' (+' . $value . ')</' . $wrap . '>';
        }
    }
    ...
    public function notifications_settings()
    {
        ...
        <div class="wpnotif_admin_head"><span><?php esc_html_e('General Settings', 'wpnotif'); ?></span></div>
            ...
            <tr>
                ...
                <td>
                    <select name="default_countrycode">
                        ...
                        <?php
                        $this->display_country_list($settings['default_countrycode']);
                        ?>
                    </select>
                </td>
            </tr>
            ...
            <tr>
                ...
                <td>
                    <select name="wpnotf_admin_user_role[]" class="wpnotif_multiselect_enable" multiple="multiple">
                        <?php
                        $manage_roles = array();
                        echo '<optgroup label="' . esc_html__('User Role', 'wpnotif') . '">';
                        foreach (wp_roles()->roles as $rkey => $rvalue) {

                            if ((!isset($rvalue['capabilities']['level_2']) || $rvalue['capabilities']['level_2'] != 1)) {
                                continue;
                            }
                            ...
                            echo '<option value="' . esc_html($rkey) . '" ' . esc_attr($ac) . '>' . esc_html($rvalue['name']) . '</option>';
                        }
                        echo '</optgroup>';

                        echo '<optgroup label="' . esc_html__('Users', 'wpnotif') . '">';
                        $users = get_users(array('fields' => array('ID'), 'role__in' => $manage_roles));
                        foreach ($users as $user_id) {
                            $user_value = 'notify_user_' . $user_id->ID;
                            $ac = '';
                            if (in_array($user_value, $admin_roles)) {
                                $ac = "selected=selected";
                            }
                            $user = get_user_by('ID', $user_id->ID);

                            echo '<option value="' . esc_html($user_value) . '" ' . esc_attr($ac) . '>' . esc_html($user->user_login) . '</option>';
                        }
                        echo '</optgroup>';
                        ?>
                    </select>
        ...
        if (class_exists('WooCommerce')) {
            ...
            <div class="wpnotif_admin_head">
                <span><?php esc_html_e('Vendor Notifications (for market place)', 'wpnotif'); ?></span></div>
            <?php
    ...
    public static function admin_header_logo($show_update = true)
    {
        ...
        <a href="https://wpnotif.unitedover.com/" target="_blank">
            <img src="<?php echo esc_html(WPNotif::get_dir('assets/images/WPNotif.svg')); ?>"
                 alt="WPNotif"/></a></h1>
        <span class="untdovr_plugin_version"><?php echo $text; ?></span>
        </a>
        <?php
    }
     ...
     * */
    public
    static function data_type($type, $data, $notify = 0)
    {
        return array('type' => $type, 'data' => $data, 'notify' => $notify);
    }

    public
    static function is_doing_ajax()
    {
