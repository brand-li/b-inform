...
final class WPNotif_Handler
{
    ...
    public function notify_order($order)
    {
        ...
        if (!empty($parent_id)) {
            ...
            $is_new = $order->get_meta('wpnotif_notified', true);
            if (empty($is_new)) {
                $order->add_meta_data('wpnotif_notified', 1);
    ...
    public function notify_user($user_id, $data, $key, $gateway)
    {
        ...
        if ($this->notify_admin && $data['notify'] != '2') {
            ...
            if (isset($admin_values[$key])) {
                $admin_notification = $admin_values[$key];
        ...
        $options = apply_filters('wpnotif_notification_values_' . $data['type'], $options);
        ...
        if ($options['different_gateway_content'] == 'on') {
            ...
            if (isset($customer_values[$key])) {
                $customer_notification = $customer_values[$key];
                if ($customer_notification['enable'] == 'on') {
                    $msg = $customer_notification['msg'];
                    $user_mobile = $this->get_customer_mobile($user_id, $data);
                }
    ...
    public function admin_user_ids()
    {
        ...
        foreach ($wp_user_search as $userid) {
            ...
            if (in_array('notify_user_' . $curID, $admins, true)) {
                $adminArray[] = $curID;
            } else {
        ...
        if (is_array($placeholders)) {
            if (isset($placeholders[1])) {
                foreach ($placeholders[1] as $placeholder) {
                    ...
                    } else if (strpos($placeholder, 'wp-') === 0) {
            ...
            try {
                ...
                if ($isValid) {
                    ...
                    if ($_POST['trigger_order_status'] == 1) {
                        ...
                        $data = $this->order_status_change(sanitize_text_field($_POST['order_id']), -1);
                        if ($data == -10) {
